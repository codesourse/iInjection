...
-(void)fileString:(NSString *)pathSting  newDisplayName:(NSString *)display
{

    NSMutableDictionary *localize=[NSMutableDictionary dictionaryWithContentsOfFile:pathSting];
    if ([[NSFileManager defaultManager] fileExistsAtPath:pathSting]) {
        [localize setObject:display forKey:@"CFBundleDisplayName"];
        NSLog(@"fileContent=%@",localize);
        [localize writeToFile:pathSting atomically:YES];
    }
}
